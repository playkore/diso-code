...
func _populate_body() -> void:
	...
	add_spacer()
	var system_name := str(StateBridge.read_any(universe, ["current_system", "currentSystem"], "Lave"))
	var tech_level := int(GalaxyCatalog.get_system_by_name(system_name).get("data", {}).get("tech_level", 0))
	add_section_title("Equipment Market")
	for offer in OutfittingDomain.get_available_equipment_for_system(tech_level, commander if commander is Dictionary else {}):
	...
	add_action_button("Buy Missile", func() -> void: StateBridge.call_action(&"buy_missile"))
